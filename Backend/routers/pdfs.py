...
@router.delete("/")
def delete_pdf(request: schemas.DeletePDFRequest, db: Session = Depends(get_db)):
    """
    PDF 삭제 엔드포인트:
    1. 데이터베이스에서 PDF 파일 정보 확인 및 삭제.
    ...
    try:
        ...
        # RunPod에 PDF 삭제 요청 전송
        session = crud.create_session(db=db, user_email=request.user_email, docs_id=request.docs_id)
        session_id = session.session_id
